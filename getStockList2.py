...
def menu():
    ...
    return select, select_market, start
...
def download_market_codelist(market):
...
def saveAlldata(filename, st, count, dbname):
    codelist = loadStockList(filename)
    totalcode = len(codelist)

    #print("시작 index : ", end="")
    #start = int(input())

    for start in range(st, totalcode, 100):
        app = QApplication(sys.argv)
        kiwoom = init_trade()

        kiwoom.setLimit(count)

        if (start<0 or start>totalcode):
            start = 0
...
if __name__ == "__main__":
    while 1:
        ...
        if not db or not market:
            exit(0)

        if db == 1 or db==3:
            download_market_codelist(market_list[market])

        if db == 2 or db == 3:
            for m in market_list[market]:
                backupDB(market_db[m])
                saveAlldata(market_lst[m], st, 200, market_db[m])
